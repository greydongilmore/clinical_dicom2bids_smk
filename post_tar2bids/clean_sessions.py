...
def get_parser():
	...
	g_req.add_argument('--dicom_dir', action='store', required=True, help='the directory with dicom data')
	g_req.add_argument('--bids_dir', action='store', required=True, help='the directory in bids format')
	
	return parser

def main(args):
	or_dates = pd.read_csv(os.path.join(args.dicom_dir, 'or_dates.tsv'), sep='\t')
	subjects = np.unique([x for x in os.listdir(args.bids_dir) if os.path.isdir(os.path.join(args.bids_dir, x)) and 'sub' in x])
	
	temp_dir = os.path.join(os.path.dirname(args.bids_dir), 'bids_final')
	if not os.path.exists(temp_dir):
		os.mkdir(temp_dir)

	bids_files = [x for x in os.listdir(args.bids_dir) if os.path.isfile(os.path.join(args.bids_dir, x))]
	for ifile in bids_files:
		shutil.copyfile(os.path.join(args.bids_dir, ifile), os.path.join(temp_dir, ifile))
		
	for isub in subjects:
		print('Converting subject {} ...'.format(isub))
		subject_or = [datetime.datetime.strptime(x, '%Y_%m_%d') for x in [y for y in or_dates[or_dates['subject']==isub.split('-')[1]]['or_date'].values] if x is not np.nan]
		orig_sessions = sorted_nicely([x for x in os.listdir(os.path.join(args.bids_dir, isub)) if os.path.isdir(os.path.join(args.bids_dir, isub, x)) and 'ses' in x])
		
		sessionDates = {'ses_num':[],'session':[]}
		for ises in orig_sessions:
			scans_tsv = [x for x in os.listdir(os.path.join(args.bids_dir, isub, ises)) if x.endswith('scans.tsv')]
			scans_data = pd.read_table(os.path.join(args.bids_dir, isub, ises, scans_tsv[0]))
			...
			for ior in subject_or:
				if dateAdded:
					if 'postsurg' in sessionDates['session'][-1]:
						if (idate-ior).days == 0:
							postop_scan=False
							for root, folders, files in os.walk(os.path.join(args.bids_dir, isub, ises)):
								for file in files:
		...
		for ilabel in sessionDates.session.unique():
			...
			for ises in sessions:
				scans_tsv = [x for x in os.listdir(os.path.join(args.bids_dir, isub, ises)) if x.endswith('scans.tsv')]
				scans_data = pd.read_table(os.path.join(args.bids_dir, isub, ises, scans_tsv[0]))
				scan_type = [x for x in os.listdir(os.path.join(args.bids_dir, isub, ises)) if os.path.isdir(os.path.join(args.bids_dir, isub, ises, x))]
				for iscan in scan_type:
					sub_path = make_bids_folders(isub, ilabel, iscan, temp_dir, True, False)
					files = [x for x in os.listdir(os.path.join(args.bids_dir, isub, ises, iscan)) if os.path.isfile(os.path.join(args.bids_dir, isub, ises, iscan, x))]
					...
					for ifile in files:
						...
						shutil.copyfile(os.path.join(args.bids_dir, isub, ises, iscan, ifile), new_file)
				...
				copytree(os.path.join(args.bids_dir, '.heudiconv', isub.split('-')[1], ises,'info'), sub_code_path)
				
			scans_file = make_bids_filename(isub, 'ses-'+ilabel, None, None, None, 'scans.json', os.path.dirname(sub_path))
			scans_json = [x for x in os.listdir(os.path.join(args.bids_dir, isub, ises)) if x.endswith('scans.json')]
			shutil.copyfile(os.path.join(args.bids_dir, isub, ises, scans_json[0]), scans_file)
