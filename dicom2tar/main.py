    ...
    try:
        if not args.clinical_scans:
            ...
            with DicomSorter.DicomSorter(dicom_dir, sort_rules.sort_rule_clinical, output_dir, args) as d:
                if os.path.exists(os.path.join(args.dicom_dir, 'errorInfo.tsv')):
                    os.remove(os.path.join(args.dicom_dir, 'errorInfo.tsv'))
                if os.path.exists(os.path.join(args.dicom_dir, 'OR_dates.tsv')):
                    os.remove(os.path.join(args.dicom_dir, 'OR_dates.tsv'))
                # tar
            ...
            ch.tarSession(output_dir, dicom_dir)
