...
def main():
	output_dir=os.path.dirname(os.path.dirname(snakemake.input.bids_fold))
	...
	#print('Converting subject {} ...'.format(os.path.basename(snakemake.input.bids_fold)))
	subject_event = []
	if snakemake.params.clinical_events:
		event_dates = pd.read_csv(snakemake.params.clinical_events, sep='\t')
		subject_event = [datetime.datetime.strptime(x, '%Y_%m_%d') for x in [y for y in event_dates[event_dates['subject']==os.path.basename(snakemake.input.bids_fold).split('-')[1]]['event_date'].values] if x is not np.nan]
	
	orig_sessions = sorted_nicely([x for x in os.listdir(snakemake.input.bids_fold) if os.path.isdir(os.path.join(snakemake.input.bids_fold, x)) and 'ses' in x])
	...
	for ises in orig_sessions:
		if not subject_event:
			...
			scans_tsv = [x for x in os.listdir(os.path.join(snakemake.input.bids_fold, ises)) if x.endswith('scans.tsv')]
			scans_data = pd.read_table(os.path.join(snakemake.input.bids_fold, ises, scans_tsv[0]))
					...
					if 'post' in sessionDates['session'][-1]:
						if abs((idate-ievent).days) <= snakemake.params.ses_calc['peri']:
							...
							if snakemake.params.ses_calc['override_peri']:
								for root, folders, files in os.walk(os.path.join(snakemake.input.bids_fold, ises)):
									for file in files:
	...
	isub = os.path.basename(snakemake.input.bids_fold)
	for ilabel in sessionDates.session.unique():
		...
		for ises in sessions:
			scans_tsv = [x for x in os.listdir(os.path.join(snakemake.input.bids_fold, ises)) if x.endswith('scans.tsv')]
			scans_data = pd.read_table(os.path.join(snakemake.input.bids_fold, ises, scans_tsv[0]))
			scan_type = [x for x in os.listdir(os.path.join(snakemake.input.bids_fold, ises)) if os.path.isdir(os.path.join(snakemake.input.bids_fold, ises, x))]
			for iscan in scan_type:
				sub_path = make_bids_folders(isub, ilabel, iscan, final_dir, True, False)
				files = [x for x in os.listdir(os.path.join(snakemake.input.bids_fold, ises, iscan)) if os.path.isfile(os.path.join(snakemake.input.bids_fold, ises, iscan, x))]
				...
				for ifile in files:
					...
					shutil.copyfile(os.path.join(snakemake.input.bids_fold, ises, iscan, ifile), new_file)
			...
			copytree(os.path.join(os.path.dirname(snakemake.input.bids_fold), '.heudiconv', isub.split('-')[1], ises,'info'), sub_code_path)
			
		scans_file = make_bids_filename(isub, 'ses-'+ilabel, None, None, None, 'scans.json', os.path.dirname(sub_path))
		scans_json = [x for x in os.listdir(os.path.join(snakemake.input.bids_fold, ises)) if x.endswith('scans.json')]
		if scans_json:
			shutil.copyfile(os.path.join(snakemake.input.bids_fold, ises, scans_json[0]), scans_file)
