    ...
    if 'DICOMDIR' not in filename:
        ...
        try:
            ...
            if dataset.Modality in {'SR', 'PR', 'KO'}:
                errorInfoTemp = "\t".join([[s for s in filename.split(os.sep) if 'sub' in s][0].split('-')[1], study_date,
            ...
            # This will skip any order sheets and localizers
                ...
                return None
            elif any(x.upper() in [z.upper() for z in list(dataset.ImageType)] for x in ('LOCALIZER')):
                return None
            else:
                    ...
                    if any(substring in modality for substring in ('Intraoperative', 'Skull', 'XA', 'RF','CR','OT')):
                        if 'CR' not in dataset.Modality:
                            if not args.clinical_events:
                                ...
                                #event_date_temp = "\t".join([args.prefix + [s for s in filename.split(os.sep) if 'sub' in s][0].split('-')[1], event_date])
                                #
                            ...
                            patient = [s for s in filename.split(os.sep) if 'sub' in s][0].split(
                                    '-')[1] + '_' + study_date
                            series_number = clean_path(
                                '{series:04d}'.format(series=dataset.SeriesNumber))
                            studyID_and_hash_studyInstanceUID = clean_path('.'.join([StudyID ,
                        ...
                        if all(x not in dataset.SeriesDescription.lower() for x in {'loc', 'dose report', 'dose record','summary'}):
